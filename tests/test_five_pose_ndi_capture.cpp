...
int runQuickCalibrationTest(std::size_t resumeFromSequenceIndex = 0) {
    ...
    try {
        ...
        waitForEnter(
            "\nTorque is still holding the arm's current position. Press "
            "Enter to disable torque and disconnect...");
        disableAll(motor, motorIds);
        motor.disconnect();
        return 0;
    } catch (const PoseSkippedByUser&) {
...
int runValidationTest(const std::string& inputCsvPath) {
    ...
    try {
        ...
        std::cout
            ...
            << "measured). Exiting with torque still enabled.\n";
        motor.disconnect();
        return 1;
    } catch (const std::exception& error) {
        std::cerr << "\nERROR: " << error.what() << '\n';
        std::cerr << "Exiting with torque still enabled.\n";
        motor.disconnect();
        return 1;
    }
}
