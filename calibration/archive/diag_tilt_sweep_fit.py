"""Jointly fit axis-tilt corrections for ALL 7 joints, on top of the
already-validated origin corrections (elbow_pitch xyz, shoulder_yaw x/z,
wrist_pitch xyz -- Y dropped from shoulder_yaw, see the degeneracy fix).
Everything free at once, fit from scratch -- no sequential-fixing bug.
SVD/condition-number check catches any NEW degeneracy before trusting it.
"""
import numpy as np
...
def fk_combined(joint_angles_rad, tilt_all, o_elbow, o_sy_xz, o_wp):
    """tilt_all: (7,2) tilt components for every joint."""
    T = np.eye(4)
...
def predict(angles_row, base_params, tilt_all, o_elbow, o_sy_xz, o_wp):
...
# layout: 19 base | 14 tilt (7x2) | 3 elbow_pitch origin | 2 shoulder_yaw origin(x,z) | 3 wrist_pitch origin
N_TILT = 14
def unpack(x):
    base_params = x[:19]
    ...
    return base_params, tilt_all, o_elbow, o_sy_xz, o_wp
...
def make_residual(angles, pos_mm, quat_xyzw):
    ...
    def residual(x):
        base_params, tilt_all, o_elbow, o_sy_xz, o_wp = unpack(x)
        params = ck.unpack_params(base_params)
        pos_res = np.zeros((n, 3)); orient_res = np.zeros((n, 3))
        for i in range(n):
            T_pred = predict(angles[i], base_params, tilt_all, o_elbow, o_sy_xz, o_wp)
...
print("=== Multi-start robustness check (all 7 tilts + 3 origin corrections jointly) ===")
...
for trial in range(n_trials):
    ...
    result = least_squares(residual_full, x0, method="lm", max_nfev=300000)
...
base_params, tilt_all, o_elbow, o_sy_xz, o_wp = unpack(last_result.x)
...
result_tr = least_squares(residual_tr, x0_tr, method="lm", max_nfev=300000)
...
print(f"\nFor reference: current validated model (wrist_pitch tilt only) = 10.80 mm")
