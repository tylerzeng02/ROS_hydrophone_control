...
# Model so far: validated wrist_pitch axis tilt + candidate elbow_pitch origin delta
WRIST_PITCH_IDX = 5
ELBOW_PITCH_IDX = 3
...
def fk_combined(joint_angles_rad, origin_delta_elbow):
    T = np.eye(4)
    for i in range(ck.N_JOINTS):
        ...
        if i == ELBOW_PITCH_IDX:
            origin = origin + origin_delta_elbow
...
def predict(angles_row, base_params, origin_delta_elbow):
    params = ck.unpack_params(base_params)
    corrected_angles = angles_row + params.joint_offsets
    T_fk = fk_combined(corrected_angles, origin_delta_elbow)
...
print("\n=== Summary ===")
print("wrist_pitch tilt only:                      RMS = 13.99 mm")
print(f"+ elbow_pitch origin (multi-start):         RMS = {np.mean(rms_results):.2f} mm")
print(f"+ elbow_pitch origin (held-out):            train={train_rms:.2f}  test={test_rms:.2f}")
