"""Idea #1 from literature research, step 2: add a periodic (1/rev Fourier)
correction directly to shoulder_pitch's own effective rotation angle --
corrected_angle = measured_angle*scale + offset + A*sin(measured_angle) +
B*cos(measured_angle) -- on top of the current best 36-param model, and
validate properly (multi-start, held-out, condition-number check for a
new degeneracy) rather than trusting the screening R^2 alone.
"""
import numpy as np
...
def predict(angles_row, base_params, joint_scales, tilt_all, o_elbow, o_sy_xz, o_wp, fourier_ab):
    ...
    # Add the periodic term to shoulder_pitch's own effective angle, using
    # its RAW measured angle (not the already-corrected one) as the phase
    # reference -- matches how the screening check was done.
...
print("=== Multi-start robustness check (36-param model + shoulder_pitch 1/rev Fourier term) ===")
...
N_EXTRA = N_SCALE + N_TILT + 3 + 2 + 3 + 2  # 31
...
print("Condition number healthy -- no new degeneracy" if cond < 1e4 else "WARNING: high condition number")

base_params, joint_scales, tilt_all, o_elbow, o_sy_xz, o_wp, fourier_ab = unpack(last_result.x)
print(f"\nFitted shoulder_pitch Fourier term: A(sin)={np.degrees(fourier_ab[0]):.3f} deg, "
      f"B(cos)={np.degrees(fourier_ab[1]):.3f} deg, amplitude={np.degrees(np.linalg.norm(fourier_ab)):.3f} deg")
...
print(f"\nFor reference: current best model without this term = 8.257 mm")
