...
// Stability-based settling wait (NdiTracker::waitForPoseStability()), used
// by every real-capture mode in this file (the default run, --quick-test,
// and --validate) rather than assuming a fixed sleep is always enough.
// Polls the moving tool until its last STABILITY_WINDOW_SIZE valid
// readings all agree within STABILITY_TOLERANCE_MM of each other.
// STABILITY_TOLERANCE_MM is set just above the ~0.3-0.4mm noise floor
// measured by --settling-diagnostic on these same poses. STABILITY_MAX_WAIT_MS
// caps how long this can run so a pose that never stabilizes doesn't block
// forever.
...
// stability wait + 30-sample average, logs every single BX poll for
...
public:
    ...
    // for durationMs, starting the instant this is called, with no wait
    // beforehand at all (not even waitForPoseStability()), and logs the
    // single-sample moving-relative-to-fixed pose at every poll. This
    // shows the actual settling curve after a move, which is what
    // justified STABILITY_TOLERANCE_MM's threshold in the first place.
...
std::vector<ValidationPoint> loadValidationPoints(const std::string& csvPath) {
    ...
    if (!in) {
        throw std::runtime_error(
            ...
            " -- run calibration/current/deployed_model_predictions.py "
...
int main(int argc, char** argv) {
    ...
    if (argc > 1 && std::string(argv[1]) == "--validate") {
        ...
        // calibration/current/deployed_model_predictions.py. Defaults to
