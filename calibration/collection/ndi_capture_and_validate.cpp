...
#ifndef _WIN32
// Linux stand-ins for Windows' <conio.h> _kbhit()/_getch(), so this file
// can build on the Linux dev machine. Puts stdin into raw, non-blocking
// mode, with no line buffering and no echo, for the process lifetime.
// Constructed once at the top of main() and restored via RAII on the way
// out. Has the same non-blocking single-key-peek semantics as the
// Windows originals: _kbhit() returns whether a key is waiting without
// consuming it, and _getch() consumes and returns it.
class LinuxRawStdin {
public:
    LinuxRawStdin() {
...
// Windows device paths. These were ported to Linux at one point for the
// ROS/MoveIt machine and reverted here for the Windows dev machine.
...
// A lower speed was needed only for the IK round-trip validation poses,
// which are generated by numerically inverting the fitted calibration
// model and were never physically hand-verified the way every other pose
// in this file was. That caution does not apply to the hand-verified
// pose sets normal runs use.
...
// If the combined position error across all joints reads exactly the
// same value for this many consecutive checks of about 100ms each, the
// arm has effectively stopped moving, stalled short of tolerance by
// friction or backlash. There is no point waiting out the full
// MOVE_TIMEOUT_SECONDS in that case, so the current position is accepted
// immediately once this triggers.
...
// moved by hand, torque off, into each pose, and the resulting joint
// ticks were captured. Every pose here is therefore a real, human-
// verified configuration rather than a randomly sampled one, so each is
// known in advance to be physically reachable and collision-free, and,
// if checked live against the "Moving tool: DETECTED/MISSING" status
// while posing, marker-visible.
const std::array<std::array<uint16_t, JOINT_COUNT>, POSE_COUNT> TARGET_POSES = {{
    ...
    // position, testing whether accuracy holds in the practical direction
    // of position to joint angles to measured result. These were never
    // hand-verified for self-collision, since they are numerically
    // inverted rather than physically posed.
    ...
    // Widened-range calibration poses: hand-posed configurations that
    // were previously excluded from the realm-restricted set above for
    // falling outside jointCalibrations' old bounds. These are now
    // included since those bounds were widened.
    // index 483-656
    ...
    // 159 poses, index 1239-1397, from recorded_hand_poses_fixed_
    // elbow_yaw(2).csv. This is a second batch of the reduced-DOF
    // (elbow_yaw locked near 2095) hand-posing session, collected for a
    // fresh calibration refit rather than an accuracy test. All 159 were
    // verified to fall within the current jointCalibrations bounds before
    // being wired in.
...
public:
    ...
    // Diagnostic only. Polls BX as fast as NDI_SAMPLE_INTERVAL_MS allows
    // for durationMs, starting the instant this is called, with no blind
    // SETTLING_TIME_MS sleep beforehand, and logs the single-sample
    // moving-relative-to-fixed pose at every poll. This shows the actual
    // settling curve after a move instead of assuming 750ms is enough
    // before the real 30-sample average starts.
    ...
    // Only the moving tool is checked, since the fixed tool never moves
    // and its stability is irrelevant here. Falls back to proceeding
    // anyway after STABILITY_MAX_WAIT_MS, consistent with this file's
    // "warn, don't abort" philosophy elsewhere (waitForBothToolsVisible,
    // collectBothTools).
...
int runQuickCalibrationTest(std::size_t resumeFromSequenceIndex = 0) {
    ...
    // A compliance slope and punch experiment was tried here and removed.
    // It caused audible vibration from too aggressive a change to both at
    // once. DynamixelMotor's punch accessors still exist if this is
    // revisited more conservatively.
    ...
    try {
        ...
        for (std::size_t i = resumeFromSequenceIndex; i < quickTestPoseCount; ++i) {
            ...
            // compensateBacklash is disabled here for the realm-restricted
            // calibration capture (TARGET_POSES 354-482), which was
            // requested uncompensated for this specific run. Restore it to
            // true for normal production runs, which is what every other
            // dataset in this project has used, including the backlash-
            // margin comparisons.
            if (!motor.moveJointsSafely(
                    motorIds,
                    targetTicks,
...
// governs precision on the actual validation points and should not be
// loosened, but this one locking move stalled a few ticks short of
// target at 5-tick tolerance, stuck at 7 ticks of error and never
// closing further. A wider margin here is harmless. The data this was
// fit on saw the same lock settle anywhere in a 25-tick window,
...
int runValidationTest(const std::string& inputCsvPath) {
    ...
    try {
        ...
        // persistent servo-firmware state, so leaving it on across
        // process exit lets a sweep script keep the arm supported across
        // many runs, disabled only once at the end via
        // disable_torque_prompt.
...
int main(int argc, char** argv) {
    ...
    if (argc > 1 && std::string(argv[1]) == "--quick-test") {
        // Optional third argument: the 1-based pose number to resume
        // from, matching the "pose N of TOTAL" numbering already printed
        // to the console. For example, `--quick-test 151` resumes right
        // after "pose 150" was last seen completed. Omit to start fresh,
        // which truncates the CSV as before.
