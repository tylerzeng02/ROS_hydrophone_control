"""Confirm the fix for the elbow_pitch/shoulder_yaw Y-origin degeneracy:
drop shoulder_yaw's origin-Y as a free parameter (fix it to 0, let
elbow_pitch's Y-origin absorb the combined effect) instead of fitting
both. If the earlier blown-up +-249000mm values were really this
structural degeneracy, removing one redundant DOF should restore a
healthy condition number and stable RMS with NO new data needed.
"""
import numpy as np
...
CSV_PATH = "C:/Users/ConformalUser/Desktop/cyton_setup/build/quick_calibration_test_287pose.csv"
...
def fk_combined(joint_angles_rad, tilt_wp, o_elbow, o_sy_xz, o_wp):
    ...
    for i in range(ck.N_JOINTS):
        ...
        if i == SHOULDER_YAW_IDX:
            # Y-component fixed at 0 -- structurally degenerate with
            # elbow_pitch's own Y-origin (see diag_verify_yaxis_degeneracy.py)
...
def unpack(x):
    # 19 base + 2 tilt + 3 elbow_pitch origin + 2 shoulder_yaw origin(x,z) + 3 wrist_pitch origin = 29
...
print("=== Multi-start check WITH shoulder_yaw origin-Y fixed to 0 (28 total params, not 30) ===")
...
print("Fitted values:")
print(f"  wrist_pitch tilt: {np.degrees(np.linalg.norm(tilt_wp)):.2f} deg")
print(f"  elbow_pitch origin (x,y,z): {np.round(o_elbow*1000, 2)} mm")
print(f"  shoulder_yaw origin (x,z only, y fixed=0): {np.round(o_sy_xz*1000, 2)} mm")
print(f"  wrist_pitch origin (x,y,z): {np.round(o_wp*1000, 2)} mm")
