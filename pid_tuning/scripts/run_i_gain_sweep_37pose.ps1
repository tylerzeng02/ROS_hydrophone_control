# Automated I-gain sweep for shoulder_pitch (motor 1), 37-pose version --
# every 10th pose from the 374-pose baseline, starting at index 0.
# Torque stays enabled across all 8 candidates -- only disabled once, at
# the very end, after an explicit Enter confirmation.
#
# Run from anywhere: .\run_i_gain_sweep_37pose.ps1
...
$inputCsv = Join-Path $DataDir "i_gain_sweep_37pose_early_subset_input.csv"
$motorId = 1
...
foreach ($i in $candidates) {
    ...
    $outFile = Join-Path $DataDir "i_gain_${i}_sweep_37pose_results.csv"
...
foreach ($i in $candidates) {
    Write-Host "  $(Join-Path $DataDir "i_gain_${i}_sweep_37pose_results.csv")"
}
